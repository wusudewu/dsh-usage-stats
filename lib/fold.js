 ...
 *
 * Moved verbatim from lib/index.js (host-side refactor split). The bodies are
 * byte-identical to the pre-split implementation; only `export` was added.
...
export function foldContextBreakdown(events) {
  ...
  function estimateSystemTokens(header) { return header?.system ? Math.ceil(header.system.length / CHARS_PER_TOKEN) + 4 : 0; }
  function estimateToolsTokens(header) { return header?.tools?.length ? Math.ceil(JSON.stringify(header.tools).length / CHARS_PER_TOKEN) + BLOCK_OVERHEAD : 0; }
...
export function foldModelUsage(events) {
  ...
  for (const event of events) {
    ...
    const stepKey = stepKeyOf(event) ?? `#fallback:${fallbackSeq++}`;
